        ...
        try {
                ...
                if (backup != null) {
                    for (Reference<?> reference : backup) {
                        changed(thread, reference,
                                ThreadLocalChangeListener.Mode.REMOVED);
                    }
                }
                return;
            }
            ...
            if (backup == null) {
                for (Reference<?> reference : current) {
                    changed(thread, reference, ThreadLocalChangeListener.Mode.ADDED);
                }
            } else {
...
// could not find it in backup - added
                        changed(thread, curRef, ThreadLocalChangeListener.Mode.ADDED);
                    }
                }
                next:
                for (Reference<?> backupRef : backup) {
